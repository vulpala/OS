	    ...
	    loff_t *f_pos)
{
  ...
  /* YOUR CODE HERE */
  dev->flag = 1;
  wake_up_interruptible(&dev->device_queue);
  printk("SLEEPY_READ DEVICE (%d): Process wakes up everyone. \n", iminor);
  /* END YOUR CODE */
	     ...
	     loff_t *f_pos)
{
  ...
  if(value > 0)
  {
       rem_jiffies = wait_event_interruptible_timeout(dev->device_queue,dev->flag == 1, HZ*value);
       retval = rem_jiffies/HZ;
  }
...
sleepy_init_module(void)
{
  ...
  for (i = 0; i < sleepy_ndevices; ++i) {
    ...
    if (err) {
     ...
     init_waitqueue_head(&(sleepy_devices[i].device_queue));
  }
